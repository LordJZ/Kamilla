...
{
    public class PoEGameProtocol : Protocol
    {
                ...
                get
                {
                    if (m_c2sStr == null)
                    {
                        ...
                        if (packet.Direction == TransferDirection.ToServer)
                            m_c2sStr = PoEGameParser.GetOpcode(packet)?.ToString() ??
                ...
                get
                {
                    if (m_s2cStr == null)
                    {
                        ...
                        if (packet.Direction == TransferDirection.ToClient)
                            m_s2cStr = PoEGameParser.GetOpcode(packet)?.ToString() ??
                                       NetworkStrings.bytes.LocalizedFormat(packet.Data.Length);
                        else
